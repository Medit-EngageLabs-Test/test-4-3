...
## The scaffold ships a demo — remove it on the first real App

This template ships with a **demonstrative sample App, "Contacts"** (backend `App/Contacts/`, frontend `App/frontend/src/app/contacts/`, its E2E `App/frontend/e2e/contacts.spec.ts`, and the `Contacts.*` roles). It exists only to show the shape of a working App — it is **not** part of the Creator's App.

**How you know it's the first time.** The signal is the demo itself: if the **Contacts** sample is still present and untouched, no real App has been built here yet — this is the first development request. Once you have removed it (below), later requests won't find it, so the signal clears itself. Don't ask the Creator whether it's the first time, and don't ask whether to keep the demo — the answer is always "remove it".

**On the first development request, remove the demo before building.** This is surgical, not `rm -rf`:

- **Delete** `App/Contacts/`, `App/frontend/src/app/contacts/`, the demo E2E `App/frontend/e2e/contacts.spec.ts`, and the `Contacts.*` role definitions.
- **Edit** the code that wired them in — the `ContactsEndpoints` registration in `Program.cs`, the `Contacts` set in `AppDbContext.cs`, `AppRoles.cs`, and `app.routes.ts` — to strip every Contacts entry.
- **Regenerate the initial migration**: `InitialCreate` defines the schema and cannot just be deleted. Drop the dev DB and re-create the migration from the cleaned model (see `AGENT-CHECKLIST.md` §7), so the App starts from its own schema, not the demo's.

If you need the Material 3 patterns the demo exemplifies, study it *first* — they are also written up in `docs/agents/conventions.md`, so nothing is lost by deleting the live code afterward.
...
| Feature | Feature branch merged into the index branch (local fast-gate passed on push; server CI runs on the release PR, not per feature) |
...
- **Prefer direct tool calls over ad-hoc scripts.** Don't write throwaway bash/Python scripts to do what a dedicated tool or a single command already does — reach for scripting only when there is no direct way. In particular, **avoid env-var-prefixed commands** like `ASPNETCORE_ENVIRONMENT=Development dotnet ef …`: set the variable on its own first (`export ASPNETCORE_ENVIRONMENT=Development`, then run the command) or pass configuration by flag. The inline-prefix form can be blocked by the command classifier and stall an autonomous run.
...
### After a push that triggers a pipeline

In this workflow the **server CI runs only on the release PR** `index/**`→`main` (see `docs/agents/workflow.md`, "Gate CI"): feature pushes to the index are gated locally by the `fast-gate` hook, not by a server pipeline. When a push *does* trigger a pipeline (the release PR, or a deploy), monitor it until completion (the `post-push-ci-watch` hook does this automatically in Claude Code sessions):
...
Repeat the push → watch → fix cycle until the pipeline is green. Do not declare the release complete while any run is pending or red. For the per-feature gate (the local fast-gate) and the park policy, see `docs/agents/workflow.md` ("Gate CI", "Politica di fallimento").

---

## Creator-facing language — always Italian

The Creator speaks Italian and is not technical. **Everything the Creator reads or is asked, you produce in Italian**: your messages in the conversation, the questions you ask, the epic draft and feature lists, the collaudo test plan, GitHub issue titles/descriptions/comments, and all business/domain documentation (the `CONTEXT.md` glossary, any business-facing docs). Business and domain terms stay in Italian — never translated.

**Never ask the Creator for an English equivalent.** Mapping an Italian domain term to an English code identifier is *your* job, done silently: you decide the identifier and record the pair in `CONTEXT.md`'s glossary (see below). Asking the Creator "how would you say X in English?" is friction and a technical question they cannot answer — it must never happen.

This axis governs what the *Creator* sees. It does not touch the code-language standard below (identifiers, comments, XML/JSDoc stay English) — that is a separate axis and stays as-is.
...
Every UI you build or modify must follow the [Material 3 guidelines](https://m3.material.io/) — M3 type scale, 8dp grid, WCAG 2.1 AA contrast/focus, a single seed-generated theme via `--mat-sys-*` tokens, and Angular Material components. The Material 3 patterns live in `docs/agents/conventions.md`; the scaffold's Contacts demo exemplifies them while it is still present (you remove it on the first real App — see "The scaffold ships a demo"). This is a platform-wide requirement, not a stylistic preference.

...
- **Implementazione** → **seriale** (una branch di lavoro per volta), **un subagent per feature** in ordine di frontiera. Il subagent è **sempre Sonnet a effort medium**: l'orchestratore lo spawna esplicitamente con `model: sonnet` ed `effort: medium`, **senza escalation di modello** (nessun ticket viene promosso a Opus). L'orchestratore fa `/grab`, spawna il subagent (implementa in TDD, committa `Closes #{ticket}`), poi `/pr`.

Il subagent parte con contesto vuoto ma **ricostruisce lo stato dall'issue** (spec nel body) e da `CONTEXT.md`/`AGENTS.md`, come dopo una compaction. L'orchestratore deve spawnarlo **con la postura del run** (pre-autorizzato; checkpoint delle skill → self-review) o fargliela rilevare dai segnali del run (index/epic branch o issue epic aperta): senza, il worker rilegge le regole generiche di conferma (CLAUDE.md globale) e si ferma a metà. Il modello del worker **non si sceglie ticket per ticket**: è sempre Sonnet-medium; la rete che intercetta i problemi prima del merge è il **fast-gate locale** (build + unit test sul lato toccato, vedi «Gate CI»), non l'escalation di modello.
...
grilling (/grill-with-docs)            intervista al Creator, nessun branch
   → /devil-advocate                   contro-esame del design (obbligatorio);
                                       finding tecnici risolti dall'agente,
                                       solo le scelte di business vanno al Creator
   ...
   → per ogni feature (seriale):
        ...
        /pr                            format (auto-fix) → PR verso la index
                                       → merge (merge commit, "Closes #feature")
                                       (niente CI per-feature: solo fast-gate locale)
                                       → branch cancellato
...
Prima della bozza, esegui `/devil-advocate` sul design: è l'ultimo momento per discuterlo — dopo l'"ok" le decisioni sono irreversibili nel run. **I finding tecnici li risolve l'agente**, adottando la risoluzione consigliata e ripiegandola nell'artefatto — non passano dal Creator. Solo i finding che cambiano **cosa fa l'App** (una scelta di prodotto/business, non di implementazione) diventano una domanda al Creator, posta in linguaggio non tecnico; "rischio accettato" è un ruling legittimo, va registrato. **Uscita**: nessun finding aperto — quelli tecnici risolti dall'agente, quelli di business chiusi da un ruling del Creator. Un finding di business senza ruling rende il GATE 1 **irraggiungibile**: niente bozza, niente "ok", finché non è chiuso. Zero finding è un esito valido: dichiaralo e procedi.
...
- **Interruzioni legittime**: i gate 2-3, un blocker esterno non rimovibile (documentalo e fermati), il park di una feature quando il fast-gate resta bloccato da un blocker esterno (vedi «Politica di fallimento»), e — transitoria — la conferma una tantum per pubblicare design su repo pubblico (vedi `issue-tracker.md`).
...
## Gate CI (completa solo a fine epica)

La CI **completa** (build + test + lint) gira **solo sulla PR di release** `index/**`→`main`: è l'unico gate CI dell'epica. Le PR di **feature** verso `index/**` **non** fanno girare la CI server-side (`.github/workflows/ci.yml`: il job `build` è condizionato a `base.ref == 'main'`); vi restano attivi solo i guard leggeri (doc-link). Il merge di una feature nella index **non attende** una CI server-side.

**Il gate per-feature è il fast-gate locale (prima del push).** L'hook `fast-gate` esegue in locale il sotto-insieme veloce dei check — format + build + unit test (backend), format + lint (frontend) — **solo sul lato toccato dal diff**, e blocca il push se falliscono. In questo regime **è il gate primario per-feature** (non più additivo alla CI): sostituisce la CI server-side sulle feature, che è rimandata alla release. I push di sola documentazione non vengono bloccati. Conseguenza esplicita: una feature entra nella index verificata solo dal fast-gate (niente E2E né suite completa cross-cutting fino alla PR di release); l'intera rete CI gira una volta, a fine epica.
...
Senza CI server-side per-feature **non c'è un contatore di "CI rosse" per feature**: il gate per-feature è il fast-gate locale, che blocca il push finché non passa. Se il fast-gate resta rosso per un **blocker esterno non rimovibile** (non un difetto del codice), vale la regola generale del run: **documenta e fermati** su quella feature, poi prosegui con la prossima **non bloccata** (chi dipende da lei resta bloccato); segnala la feature come parcheggiata (label `parked` idempotente + commento con la diagnosi) alla consegna del collaudo.

La CI **completa** ricompare solo alla **PR di release** `index/**`→`main`. Se quella è rossa, si corregge sul branch `index/**` e si ri-pusha (come nell'epica semplice): è l'unico punto in cui una CI server-side può bloccare, ed è a fine epica.

## Collaudo

**Code-review obbligatoria prima della consegna.** Prima di avviare lo stack e consegnare al Creator, esegui `/code-review` sul diff dell'intero turno di sviluppo (la index rispetto a `main`), con i subagent di revisione su **`opus` a effort medio/alto** (non `xhigh`). I rilievi bloccanti si risolvono prima della consegna; il ciclo review→fix rispetta la politica di park. La review rivede il **diff** (Standards + Spec); è complementare all'advisor, che durante il run rivede l'approccio. Si esegue **una volta per turno di sviluppo** (prima consegna e ogni ciclo di feedback), non per singola feature — il gate per-feature resta il **fast-gate locale** (la CI completa gira solo alla release).
...
5. messaggio al Creator: URL + **piano di test funzionale** (sotto) + feature parcheggiate + debito dipendenze major (`release.md` §2);
6. stack acceso per tutta la durata dei feedback.

**Piano di test funzionale (con riferimenti).** La checklist di collaudo è un **piano di test funzionale** in linguaggio non tecnico, **raggruppato per area/feature** con una lettera (A, B, C…) e ogni verifica **numerata** dentro il gruppo (A1, A2, A3, B1, B2, C1…). Ogni voce è un'azione concreta che il Creator esegue sull'App girante, col risultato atteso. I riferimenti non sono decorativi: servono al Creator per **citare il singolo caso nel feedback** ("A2 non funziona", "B1 ok") — così un'osservazione di collaudo si aggancia a un punto preciso del piano, e la nuova feature che ne nasce (vedi "Cicli di feedback") parte già localizzata.

...
- **Angular Material is the reference UI library** and is already installed and
  ...
  - Prefer Material components over browser primitives (`MatDialog` instead of
    ...
    `alert()`).
  - These patterns (toolbar, `mat-table`, `mat-form-field` form, confirm dialog)
    are exemplified by the scaffold's Contacts demo *while it is still present* —
    the demo is removed on the first real App (see `AGENTS.md`, "The scaffold
    ships a demo"). Treat this write-up, not the live demo, as the durable

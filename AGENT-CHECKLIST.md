...
> **These are reference commands — run them directly.** Don't wrap them in throwaway helper scripts, and don't chain them behind env-var prefixes (`VAR=value cmd`, e.g. `ASPNETCORE_ENVIRONMENT=Development dotnet ef …`): set any variable on its own line first (`export …`), or pass it by flag. The inline-prefix form can trip the command classifier and stall an autonomous run. Keep each step a single, direct command.
...
4. For any UI, use Angular Material with the pre-installed theme (see the
   "UI guidelines" section of `AGENTS.md`); the Material 3 patterns are written up in
   `docs/agents/conventions.md` — the Contacts demo exemplifies them only while still
   present (it is removed on the first real App).
...
5. Deliver to the Creator: the URL to open, a **functional test plan** in non-technical language — grouped by area/feature (A, B, C…) with each check numbered inside its group (A1, A2, A3, B1, B2, C1…) so the Creator can cite a single case in feedback ("A2 non funziona") — parked features declared with their diagnosis, and the major dependency-debt summary (`release.md` §2).
...
2. **Structured logs** — every domain operation emits a structured log entry with an
   `EventId`; filter by event name. The scaffold's Contacts demo illustrated the
   convention (e.g. `ContactsListed` (1001), `ContactCreated` (1004), `ContactNotFound`
   (1003, **Warning**), `ContactDeleted` (1006)); once the demo is removed, your App
   defines its own events the same way — one per meaningful operation, numbered in a
   per-area `EventId` range.
